...
2. Create your commit using `git commit -m "type(TTP-XX): your message"`
   - *Note: If the user did not provide a specific ticket number in the conversation, use a placeholder ticket like `TTP-00` or ask them what ticket number they are working on before committing.*
3. **DO NOT PUSH.** Under the core project rules, you are explicitly forbidden from running `git push` automatically. You may only push if the user explicitly says "push this" or "push to remote".
