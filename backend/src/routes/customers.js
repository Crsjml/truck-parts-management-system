...
router.get('/', requireAuth, requireRole('ADMIN'), async (req, res) => {
  try {
    ...
    for (const tx of transactions) {
      if (tx.userId && validAuthIds.includes(tx.userId) && !tx.userId.startsWith('temp-')) continue;
...
router.get('/:id/transactions', requireAuth, requireRole('ADMIN'), async (req, res) => {
  try {
    ...
    if (uuidRegex.test(id)) {
      ...
      whereClause = isEmail
        ? { customerEmail: { equals: key, mode: 'insensitive' }, userId: null }
        : { customerContact: key, userId: null };
    } else {
